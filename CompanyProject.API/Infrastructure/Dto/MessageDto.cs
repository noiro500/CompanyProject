...
namespace CompanyProject.API.Infrastructure.Dto
{
    public class MessageDto
    {
        ...
        [Display(Name = "Ваше имя: *")] public string PeopleName { get; set; } = null!;
        ...
        [Display(Name = "Тема сообщения: *")] public string SubjectMessage { get; set; } = null!;
        ...
        public class MessageValidator : AbstractValidator<MessageDto>
        {
            ...
            {
                RuleFor(x => x.PeopleName).NotEmpty().WithMessage("Не указано имя").Matches(@"^[а-яА-Я0-9._%+""'\s-]*$")
                ...
                RuleFor(x => x.SubjectMessage).NotEmpty().WithMessage("Не указана тема сообщения")
                    .Matches(@"^[а-яА-Я0-9._%+""'\s-]*$")
                    .WithMessage("Некорректные символы. Допускаются только русские символы!")
                    .MaximumLength(50).WithMessage("Максимальная длина темы - 50 символов");
                RuleFor(x => x.Content).NotEmpty().WithMessage("Не введено сообщение")
                    .Matches(@"^[а-яА-Я0-9._%+""'\s-]*$")
                    .WithMessage("Некорректные символы. Допускаются только русские символы!")
